...
class Parser {
    constructor(tokens) {
        this.actual = 0; // índice del token que estamos viendo
        this.tokens = tokens;
    }
    ...
    analizar() {
        ...
        if (this.actual < this.tokens.length) {
            ...
            throw new Error(`Token inesperado '${sobrante.valor}' en la posición ${sobrante.posicion + 1}`);
        }
        return arbol;
    }
    ...
    primario() {
        ...
        throw new Error(`Se esperaba un número, variable o '(' pero se encontró '${token.valor}' en la posición ${token.posicion + 1}`);
    }
...
function recorrerPreorden(nodo) {
